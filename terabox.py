...
def get_data(url: str):
    ...
    data = {
        "file_name": (
            list.get("server_filename")
            if r_j.get("list") and r_j["list"] and list
            else None
        ),
        "link": (
            list.get("dlink") if r_j.get("list") and r_j["list"] and list else None
        ),
        "direct_link": direct_link,
        "thumb": (
            list["thumbs"].get("url3")
            if r_j.get("list") and r_j["list"] and list and "thumbs" in list
            else default_thumbnail if default_thumbnail else None
        ),
        "size": (
            get_formatted_size(int(r_j["list"][0]["size"]))
            if r_j.get("list") and r_j["list"] and list
            else None
        ),
        "sizebytes": (
